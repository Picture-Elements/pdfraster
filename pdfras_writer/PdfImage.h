...
extern t_pdvalue pd_make_calgray_colorspace(t_pdallocsys *alloc, double black[3], double white[3], double gamma);

// Create & return a standard sRGB colorspace (as an ICCBased colorspace).
// The ICC profile is per IEC61966-2.1
extern t_pdvalue pd_make_srgb_colorspace(t_pdallocsys *alloc, t_pdxref *xref);
...
extern t_pdvalue pd_make_iccbased_rgb_colorspace(t_pdallocsys *alloc, t_pdxref *xref, const pduint8* prof_data, size_t prof_size);
